...
const ForgotPage = () => {
	...
	const handleLogin = async (e) => {
		...
		try {
			...
			if (res.status === 200) {
				showToast('Yêu cầu đặt lại mật khẩu thành công! Vui lòng kiểm tra email của bạn.', 'success');

				setTimeout(() => {
					navigator(ROUTES.LOGIN);
				}, 3000);
			} else {
				showToast(res.message || 'Yêu cầu đặt lại mật khẩu thất bại. Vui lòng thử lại.', 'error');
			}
		} catch (err) {
			showToast('Yêu cầu đặt lại mật khẩu thất bại. Vui lòng thử lại.', 'error');
		} finally {
			setLoading(false);
		}
	};
