...
// import './salerHeader.css';

const SalerHeader = ({ toggleSidebar }) => {
    ...
    const generateBreadcrumbs = () => {
        ...
        // Always add Home
        breadcrumbs.push({ name: 'Trang chủ', path: '/saler' });
        
        // Add path segments
        let currentPath = '';
        paths.forEach((path, index) => {
            if (path === 'saler') return; // Skip saler in breadcrumb
            
            currentPath += `/${path}`;
            const pathName = pathMappings[path] || path.charAt(0).toUpperCase() + path.slice(1);
            breadcrumbs.push({
                name: pathName,
                path: index === paths.length - 1 ? null : `/saler${currentPath}`
            });
        });
