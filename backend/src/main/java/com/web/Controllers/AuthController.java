...
import com.web.Config.JwtUtil;
import com.web.Dto.LoginRequest;
import com.web.Model.AuthResponse;
import com.web.Model.ResetToken;
import com.web.Model.User;
import com.web.Repository.ResetTokenRepository;
...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request) {
        ...
        } else if ("Chưa kích hoạt".equals(user.getStatus())) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body("Tài khoản của bạn chưa được kích hoạt. Vui lòng kiểm tra email để kích hoạt tài khoản.");
        }
    ...
    public ResponseEntity<String> register(@RequestBody User user) {
        if (user.getPhone() == null || user.getPhone().isEmpty() || user.getEmail() == null || user.getEmail().isEmpty()) {
            return new ResponseEntity<>("Không được bỏ trống Số điện thoại, Email.", HttpStatus.BAD_REQUEST);
        }
        String token = jwtUtil.generateResetToken(user.getEmail(), 1440L);
        ...
        User registeredUser = userService.registerUser(user, token);
    ...
    public ResponseEntity<?> repass(@RequestBody User user) {
        String token = jwtUtil.generateResetToken(user.getEmail(), 5L);
        ResetToken resetToken = new ResetToken();
        resetToken.setEmail(user.getEmail());
        resetToken.setToken(token);
        resetToken.setUsed(false);
